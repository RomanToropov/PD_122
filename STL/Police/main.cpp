...
void main()
{
	...
	{
		std::pair<std::string, std::list<Crime>>("m777ok",{Crime(1, "ул. Ленина"), Crime(2, "ул. Ленина"), Crime(8, "Перекресток Ленина и Октябрьской")}),
	...
	if (fin.is_open())
	{
		...
		while (!fin.eof())
		{
			...
			if (all_crimes.find(',') == std::string::npos)
			{
				...
				std::string place = all_crimes.erase(0, 1);
